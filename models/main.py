...
def decode_and_sample(
    ...
    features = tf.parse_single_example(
        ...
        features={
            ...
            "actions": tf.FixedLenFeature([sampling_max_horizon * act_dim], tf.float32)
            if "fetch" not in task
            else tf.FixedLenFeature([sampling_max_horizon * (act_dim + 1)], tf.float32),
    ...
    t1 = tf.random_uniform(
        shape=[], minval=0, maxval=sampling_max_horizon + 1 - dt, dtype=tf.int32
    )
    t2 = t1 + dt
    ...
    atT = tf.concat(
        values=[
            (1.0 / act_scale_coeff) * actions[t1:t2],
            tf.zeros((max_horizon - effective_horizon, act_dim)),
        ],
        axis=0,
    )
    qtT = qt[t1]
    mask = tf.concat(
        values=[
            tf.ones((effective_horizon)),
            tf.zeros((max_horizon - effective_horizon)),
        ],
        axis=0,
    ) + tf.zeros((max_horizon))
...
def main():
    ...
    dirname = (
        ...
        + str(args.il_lr)
        + "_num_plan_updates_"
        + str(args.num_plan_updates)
        + "_horizon_"
        + str(args.inner_horizon)
        + "_num_train_"
        + str(args.num_train)
        + "_"
        + decay_str
        ...
        + learn_lr_str
        + "_clip"
        + str(args.meta_gradient_clip_value)
        + "_n_hidden_"
        + str(args.n_hidden)
        + "_latent_dim_"
        + str(args.obs_latent_dim)
        + "_dt_"
        + str(args.dt)
    )
    ...
    if args.bias_transform:
        dirname += "_bt"
    dirname += (
        "_n_act_%d" % args.n_hidden_act + "_act_latent_dim_" + str(args.act_latent_dim)
    )
    ...
    if not test:
        ...
        filenames = [glob.glob(args.training_tfrecord)] + [
            glob.glob(args.validation_tfrecord)
        ]
        iterators = []
        for filename in filenames:
            ...
            dataset = dataset.repeat(
                count=args.num_batch_updates // (iter_per_epoch) + 2
            )
            dataset = dataset.shuffle(500 + 3 * args.batch_size)
            dataset = dataset.map(
                lambda example: decode_and_sample(
                    example,
                    img_h=args.img_h,
                    img_w=args.img_w,
    ...
    if test:
        try:
            model_file = tf.train.latest_checkpoint(
                log_directory + "summ/" + dirname + "/models/"
            )
        except:
            import glob
            ...
            model_file = (
                ...
                + "summ/"
                + dirname
                + "/models/model_%d" % args.restore_iter
            )
        if args.restore_iter > 0:
            model_file = (
                model_file[: model_file.index("model_")]
                + "model_"
                + str(args.restore_iter)
            )
        if model_file:
        ...
        train_writer = tf.summary.FileWriter(
            log_directory + "summ/" + dirname, sess.graph
        )
        for batch_idx in range(args.num_batch_updates):
            imp_networks[0].train(args.il_lr_0, args.il_lr, args.ol_lr, sess)
            ...
            if batch_idx % args.testing_frequency == 0:
                (
                    ...
                    latent_xpred_t,
                    latent_xg_t,
                    bc_loss_first_step_train,
                    train_summ,
                ) = imp_networks[0].stats(args.il_lr_0, args.il_lr, sess)
                ...
                (
                    ...
                    bc_loss_first_step,
                    val_summ,
                ) = imp_networks[1].stats(args.il_lr_0, args.il_lr, sess)
